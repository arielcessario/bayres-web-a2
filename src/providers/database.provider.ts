...
import {DatabaseConnectorProvider} from './database-connector.provider';
...
export class DatabaseService {
    ...
    static af: AngularFireDatabase;


    notification$: Observable<any>;
    private observer: Observer<any>;
    filter$: Observable<any>;
    observerFilter: Observer<any>;
    ...
    constructor(protected fireCache: DatabaseConnectorProvider) {
        ...
        //     this.cache[this.name] = FireFactoryService.af.list('/' + this.name, {
        //         query: {
        //             orderByKey: true
        //         }
        //     });
        ...
        this.filter$ = new Observable(observer => this.observerFilter = observer).share();
    ...
    get(params?: any) {
        ...
        let con = this.urls[this.name] + '?' + str;

        let cache = this.cache.get(this.name);
        if (cache != undefined) {
            return new BehaviorSubject(cache);
        }
        return this.fireCache.get(con, this.name);
    }
     ...
     */
    update(form: any, fnc: string, obj: string) {

        let ret = this.parseForm(form).obj;
        ...
        return this.fireCache.create(params, obj);
    }
        ...
        ret.items.push(ret.obj).then(res => {
            ...
            for (let i in index_keys) {
                ...
                for (let x in _keys) {
                    ...
                    // FireFactoryService.af.object('/' + index_keys[i] + '/' + _keys[x] + '/' + this.name).update(val);
                }
            }
        });
    ...
    createRaw(data: any, model: string) {
        this.fireCache.create(data, this.urls[model]);
    }
    ...
    remove(item: any, form: any) {
        ...
        // let ret = this.parseForm(form);
        //
        // const itemObservable = FireFactoryService.af.object(this.name + '/' + item.$key);
        ...
        //     for (let x in _keys) {
        //         FireFactoryService.af.object('/' + index_keys[i] + '/' + _keys[x] + '/' + this.name + '/' + item.$key + '/').remove();
        //     }
        // }
        //
        // this.resetForm(form);
    ...
    join(params: string[]) {
        ...
        //             // la respuesta)
        //             for (var x in _ids) {
        //                 response[i][params[y]].obs.push(FireFactoryService.af.object('/' + params[y] + '/' + [_ids[x]]));
        //             }
        //
        //         }
        //     }
     ...
     * @param form
     */
    selectItem(key: string, form: any, data: any) {
            ...
            if (form.controls[props[i]] != undefined) {
                ...
                if (form.controls[props[i]].controls instanceof Array) {
                    for (var x in form.controls[props[i]].controls) {
                        ...
                        let _key = form.controls[props[i]].controls[x].controls['id'].value;
    ...
    filter(params, values, exact_match) {

        this.get(this.name).subscribe(data => {
            if (data.length == 0) {
                return;
            }
            ...
            let exactos = exact_match.split(',');

            var respuesta = [];
            for (var y = 0; y < data.length; y++) {
                let columns = Object.keys(data[y]);
                ...
                for (var i = 0; i < columns.length; i++) {
                    for (var x = 0; x < parametros.length; x++) {
                        if (columns[i] == parametros[x]) {
                            ...
                            if (negado) {
                                if (
                                    ( exacto && this.removeDiacritics(base).toUpperCase() !== this.removeDiacritics(valor).toUpperCase().replace('!', '')) ||
                                    (!exacto && !this.compareStringSearch(base.replace('!', ''), valor))
                                ...
                                if (
                                    ( exacto && this.removeDiacritics(base).toUpperCase() == this.removeDiacritics(valor).toUpperCase()) ||
