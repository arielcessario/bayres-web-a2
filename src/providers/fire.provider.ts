...
export class FireFactoryService {
     ...
     * @param f [description]
     */
    static init(af: AngularFireDatabase) {
    ...
    parseForm(form: any): any {
        const items = FireFactoryService.af.list(this.name);
        const indexes = {};
    ...
    selectItem(key: string, form: any) {
        FireFactoryService.af.object('/' + this.name + '/' + key).subscribe(function (data) {
    ...
    filter(params, values, exact_match) {
        this.fireCache.get(this.name).subscribe(data => {
            if (data.length == 0) {
                return;
            }
